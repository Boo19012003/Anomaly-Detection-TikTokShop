...
async def upsert_to_supabase(structured_data):
    ...
    try:
        ...
        if structured_data.get("shops"):
            ...
            logger.info(f"Đã giả lập đẩy {len(structured_data['shops'])} shops lên DB.")
            
        # Upsert Products
        if structured_data.get("products"):
            supabase.table("products").upsert(structured_data["products"]).execute()
            logger.info(f"Đã giả lập đẩy {len(structured_data['products'])} products lên DB.")
        ...
        if structured_data.get("products_metrics_history"):
            ...
            logger.info(f"Đã giả lập đẩy {len(structured_data['products_metrics_history'])} bản ghi metrics lên DB.")
        ...
        if structured_data.get("reviews"):
            ...
            logger.info(f"Đã giả lập đẩy {len(structured_data['reviews'])} reviews lên DB.")
...
async def crawler():
    async with async_playwright() as p:
        ...
        url_list = [
            "https://www.tiktok.com/shop/vn/pdp/sevenlab-gel-cham-mun-anti-acne-cream-retinol-1-pionin/1733149425459496223",
            # "https://www.tiktok.com/shop/vn/pdp/set-4-ban-chai-%C4%91anh-rang-3-mat-long-nhe-de-thay-the/1732068819765200373",
            # "https://www.tiktok.com/shop/vn/pdp/demejine-korean-day-night-cream-prevents-acne-evens-skin-tone/1732784849692296537",
            # "https://www.tiktok.com/shop/vn/pdp/kem-chong-nang-cho-da-nhay-cam-spf-50-50ml-madagascar/1733036943667594852",
            # "https://www.tiktok.com/shop/vn/pdp/set-3-dao-cao-long-may-tien-dung-thiet-ke-gap-gon-an-toan-hieu-qua/1731825056217466252",
            # "https://www.tiktok.com/shop/vn/pdp/combo-5-bich-bong-tay-trang-cotton-tu-nhien-222-mieng-moi-goi/1730059270274648375",
            # "https://www.tiktok.com/shop/vn/pdp/cap-mat-gia-tap-noi-mi-chuyen-dung-cho-hoc-vien-sieu-thuc/1732174137536906713",
            # "https://www.tiktok.com/shop/vn/pdp/combo-judydoll-bang-phan-kem-lot-duong-da-khoang-cho-nu/1731163992815207991",
            # "https://www.tiktok.com/shop/vn/pdp/kem-u-toc-lambor-500ml-phuc-hoi-toc-hu-ton-nuoi-duong-mem-muot/1731860892331313703",
            # "https://www.tiktok.com/shop/vn/pdp/1734013597279749558"
        ]

        logger.info(f"Bắt đầu trích xuất chi tiết {len(url_list)} sản phẩm...")
        
        # Dữ liệu trả về lúc này đã là data tinh gọn, không phải JSON thô
        structured_data = await process_request_product(context, url_list, 5)

        # Lưu file để bạn kiểm tra cấu trúc trước khi mở khóa DB
        # output_file = "extracted_tiktok_data.json"
        # with open(output_file, "w", encoding="utf-8") as f:
        #     json.dump(structured_data, f, ensure_ascii=False, indent=4)
            
        # logger.info(f"Đã xuất file dữ liệu hoàn chỉnh: {output_file}")
